...
#define OPTION(t, p) {t, offsetof(struct minifs_file, p), 1}
static const struct fuse_opt option_spec[] = {
    OPTION("--name=%s", name), OPTION("--contents=%s", data), FUSE_OPT_END};

static void *minifs_init(struct fuse_conn_info *conn, struct fuse_config *cfg) {
  (void)conn;
  cfg->kernel_cache = 1;
  return NULL;
}
...
int main(int argc, char *argv[]) {
  ...
  options2.data =
      strdup("This filesystem has implemented the getattr (similar to stat), "
             "readdir, open, and read \n"
